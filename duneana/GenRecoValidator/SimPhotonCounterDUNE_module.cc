// \file SimPhotonCounterDUNE_module.cc
// \author Ben Jones, MIT 2010 - edited by Daniele Guffanti, INFN Milano-Bicocca 2026
//
// Extended version of the larana/SimPhotonCounter_module.cc 
// to determine how many phots have been detected at each OpDet
//
// This analyzer takes the SimPhotonsCollection generated by LArG4's sensitive detectors
// and fills up to five trees in the histograms file.  The five trees are:
//
...
// OpDetsGeo        - placement, size and associated channels of each OpDet
//
...
// [string] InputModule - the modules which produced the SimPhotonsCollection
// [int]    InputModuleCode - an array of int encoring the respective InputModule
...
// bool     MakeOpDetsGeoTree
// bool     IncludePhotonTile - Store photon hit time profile in the OpDets tree
...
namespace opdet {
  ...
  private:
    /// Fill opDets geometry ttree
    void fill_opdet_geo_tree();
    ...
    TTree* fTheOpDetTree;
    TTree* fTheOpDetGeoTree;
    TTree* fTheEventTree;

    // Parameters to read in

    std::vector<std::string> fInputModule; // Input tag for OpDet collection
    std::vector<int> fInputModuleCode = {}; // Input code to be stored in output
    ...
    Int_t fNTickTime = 0;
    std::vector<int> fPhotonTickTime = {}; 
    std::vector<int> fNPhotonsPerTick = {};
    Int_t fEventID;
    Int_t fOpChannel;

    // OpDet geo information
    Float_t opDetH = 0.0; //! OpDet height
    ...
    int fRun, fTrackID, fpdg, fmotherTrackID, fsimphModuleCode;
...
namespace opdet {
    ...
    , fUseLitePhotons(art::ServiceHandle<sim::LArG4Parameters const>()->UseLitePhotons())
  {
    ...
    try {
      fInputModuleCode = pset.get<std::vector<int>>("InputModuleCode");
    }
    catch (...) {
      fInputModuleCode = {};
    }
  ...
  void SimPhotonCounter::beginJob()
  {
    ...
    if (fMakeOpDetsTree) {
      ...
      fTheOpDetTree->Branch("InputModuleCode", &fsimphModuleCode, "InputModuleCode/I");
      ...
      if (fIncludePhotonTime) {
        fTheOpDetTree->Branch("TickTime", &fPhotonTickTime);
        fTheOpDetTree->Branch("NPhotonsPerTick", &fNPhotonsPerTick);
      }
    }

    if (fMakeOpDetsGeoTree) {
      fTheOpDetGeoTree = tfs->make<TTree>("OpDetsGeo", "opDetMap");
      fTheOpDetGeoTree->Branch("opDetH", &opDetH); 
    ...
    if (fMakeOpDetEventsTree) {
      ...
      //if (fPVS->StoreReflected()) // FIXME: Is this a bug?
        //fTheOpDetTree->Branch("CountReflDetected", &fCountOpDetReflDetected, "CountReflDetected/I");
    }
  ...
  void SimPhotonCounter::analyze(art::Event const& evt)
  {
    ...
    if (fUseLitePhotons) {
      ...
      int imod = 0;
      for (auto const& mod : fInputModule) {
        fsimphModuleCode = (fInputModuleCode.empty()) ? 0 : fInputModuleCode.at(imod);
        ...
        for (auto const& ph_handle : photon_handles) {
          ...
          if ((*ph_handle).size() > 0) {
            ...
            for (auto const& photon : (*ph_handle)) {
              ...
              fPhotonTickTime.clear();
              fNPhotonsPerTick.clear();
              fNTickTime = 0;
              ...
              for (auto it = PhotonsMap.begin(); it != PhotonsMap.end(); it++) {
                ...
                //Reset photon time vectors

                //Get arrival time from phot
                fTime = it->first;

                fNTickTime++;
                fPhotonTickTime.emplace_back( it->first );
                fNPhotonsPerTick.emplace_back( it->second );
        ...
        } // loop over ph_handles
        imod++;
      } // loop over fInputModule
    }
